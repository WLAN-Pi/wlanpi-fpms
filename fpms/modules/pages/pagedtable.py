...
class PagedTable(object):
    ...
    def display_paged_table(self, g_vars, table_data, justify=True):
        ...
        # Draw title
        g_vars["draw"].rectangle(
            (x, y, PAGE_WIDTH, STATUS_BAR_HEIGHT),
            fill=THEME.page_table_title_background.value,
        )
        title_size = SMART_FONT.getbbox(title)
        g_vars["draw"].text(
            (x + (PAGE_WIDTH - title_size[2]) / 2, y + font_offset),
            title,
            font=SMART_FONT,
            fill=THEME.page_table_title_foreground.value,
        )

        # Draw back nav indicator
        g_vars["draw"].line(
            [(2, (STATUS_BAR_HEIGHT / 2)), (6, 4)],
            fill=THEME.page_table_title_foreground.value,
            width=1,
        )
        g_vars["draw"].line(
            [(2, (STATUS_BAR_HEIGHT / 2)), (6, STATUS_BAR_HEIGHT - 4)],
            fill=THEME.page_table_title_foreground.value,
            width=1,
        )
        ...
        if multi_page:
            ...
            up_fill_color = (
                THEME.page_table_disabled_title_foreground.value
                if current_page == 1
                else THEME.page_table_title_foreground.value
            )
            down_fill_color = (
                THEME.page_table_disabled_title_foreground.value
            ...
            g_vars["draw"].line(
                [(PAGE_WIDTH - 7, 2), (PAGE_WIDTH - 3, (STATUS_BAR_HEIGHT / 2) - 2)],
                fill=up_fill_color,
                width=1,
            )
            g_vars["draw"].line(
                [(PAGE_WIDTH - 7, 2), (PAGE_WIDTH - 11, (STATUS_BAR_HEIGHT / 2) - 2)],
                fill=up_fill_color,
                width=1,
            )
            ...
            g_vars["draw"].line(
                [
                    (PAGE_WIDTH - 7, STATUS_BAR_HEIGHT - 2),
                    (PAGE_WIDTH - 3, (STATUS_BAR_HEIGHT / 2) + 2),
                ],
                fill=down_fill_color,
                width=1,
            )
            g_vars["draw"].line(
                [
                    (PAGE_WIDTH - 7, STATUS_BAR_HEIGHT - 2),
                    (PAGE_WIDTH - 11, (STATUS_BAR_HEIGHT / 2) + 2),
                ],
                fill=down_fill_color,
                width=1,
            )
        ...
        if page_count > 0:
            ...
            oled.render_text(
                f"{title} [{current_page}/{total_pages}]", page
            ) if multi_page else oled.render_text(title, page)
            ...
            for item in page:
                ...
                if item == "---":
                    g_vars["draw"].line(
                        [(x, y + font_offset + 2), (PAGE_WIDTH, y + font_offset + 2)],
                        fill=THEME.page_table_row_separator.value,
                    )
                    font_offset += 3
                else:
                    g_vars["draw"].text(
                        ...
                        item,
                        font=SMART_FONT,
                        fill=THEME.page_table_row_foreground.value,
                    )
        ...
        if multi_page:
            ...
            g_vars["draw"].line(
                [(x, y), (x, y + scroll_bar_length)],
                fill=THEME.page_table_scrollbar.value,
                joint="curve",
            )
    ...
    def display_empty_page(self, g_vars, title="", footer=""):
        ...
        g_vars["draw"].text(
            ((PAGE_WIDTH - footer_size[2]) / 2, PAGE_HEIGHT - 20),
            footer,
            font=SMART_FONT,
            fill=THEME.page_table_row_foreground.value,
        )
