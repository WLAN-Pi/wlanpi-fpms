...
class Alert:
    ...
    def display_alert(
        ...
        # write title
        self.draw.rectangle(
            (x, y, PAGE_WIDTH, STATUS_BAR_HEIGHT), fill=title_background
        )
        ...
        self.draw.text(
            ...
            title,
            font=SMART_FONT,
            fill=title_foreground,
        )
        font_offset += font_size

        # draw back nav indicator
        g_vars["draw"].line(
            [(4, (STATUS_BAR_HEIGHT / 2)), (8, 4)], fill=title_foreground, width=1
        )
        g_vars["draw"].line(
        ...
        for item in item_list:
            ...
            self.draw.text(
                (x + (PAGE_WIDTH - item_size[2]) / 2, y + font_offset),
                item,
                font=font_type,
                fill=THEME.alert_message_foreground.value,
            )
    ...
    def display_popup_alert(self, g_vars, msg, delay=0.5):
        ...
        self.draw.rectangle(
            (margin, y - (rect_height / 2), PAGE_WIDTH - margin, y + rect_height),
            outline=THEME.alert_popup_foreground.value,
            fill=THEME.alert_popup_background.value,
        )
        ...
        for item in item_list:
            ...
            self.draw.text(
                (x + (PAGE_WIDTH - text_size[2]) / 2, y + font_offset),
                item,
                font=SMART_FONT,
                fill=THEME.alert_popup_foreground.value,
            )
            font_offset += font_size
