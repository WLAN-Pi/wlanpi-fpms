    ...
    def check_for_updates(self, g_vars):
        if not g_vars["result_cache"]:
            ...
            try:
                ...
                output = subprocess.check_output(
                    "/usr/bin/wlanpi-update", shell=True
                ).decode()
                new_packages = (
                    subprocess.check_output(
                        "echo '"
                        + output
                        + "' | grep ^wlanpi- | awk '{ print $1\":\"$2 }' | sed 's/\\/.*:/:/g'",
                        shell=True,
                    )
                    .decode()
                    .strip()
        ...
        if updates is None:
            ...
            self.simple_table_obj.display_simple_table(
                g_vars, g_vars["updates"], title="Updates found"
            )
        else:
            self.alert_obj.display_alert_info(
                g_vars, "All WLAN Pi packages are up-to-date.", title="No updates found"
            )
        ...
        if not g_vars["result_cache"]:
            ...
            try:
                ...
                try:
                    if len(new_packages) > 0:
                        self.alert_obj.display_popup_alert(
                            g_vars, "Installing updates, please wait..."
                        )
                        subprocess.check_output(
                            "/usr/bin/wlanpi-update -u", shell=True
                        ).decode().strip()
                        self.alert_obj.display_alert_info(
                            g_vars, "Packages updated successfully.", title="Success"
                        )
                    else:
                        self.alert_obj.display_alert_error(
